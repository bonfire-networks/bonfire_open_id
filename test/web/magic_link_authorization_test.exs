defmodule Bonfire.OpenID.Web.MagicLinkAuthorizationTest do
  ...
  setup tags do
    ...
    client = ClientApps.init_test_client_app(Faker.UUID.v4(), %{
      name: "Magic link regression", redirect_uris: [callback],
      pkce: tags[:pkce] || false, supported_scopes: ["read", "write", "openid", "profile"]
    })
  ...
  for protocol <- ["oauth", "openid"] do
    @protocol protocol
    @tag pkce: true
    test "#{protocol}: magic-link Allow preserves PKCE, nonce and state through token exchange", c do
      verifier = String.duplicate("magic-link-proof-", 4)
      challenge = :crypto.hash(:sha256, verifier) |> Base.url_encode64(padding: false)
      params = params(c, @protocol) |> Map.merge(%{"code_challenge" => challenge,
        "code_challenge_method" => "S256", "nonce" => "nonce & + / café"})
      login = redeem(c, @protocol, params)
      assert login.status == 200
      {:ok, view, _} = live(login)
      assert {:error, {:live_redirect, %{to: next}}} = view |> element("[data-role=oauth_consent_allow]") |> render_click()
      ...
      response = conn() |> post("/#{@protocol}/token", %{"grant_type" => "authorization_code",
        "client_id" => c.client.id, "client_secret" => c.client.secret,
        "code" => returned["code"], "redirect_uri" => c.callback, "code_verifier" => verifier}) |> json_response(200)
      assert is_binary(response["access_token"])
    end

    @tag callback_query: "?destination=inbox"
    test "#{protocol}: magic-link Deny preserves callback query and state without granting consent", c do
      login = redeem(c, @protocol, params(c, @protocol))
      {:ok, view, _} = live(login)
      assert {:error, {:redirect, %{to: next}}} = view |> element("[data-role=oauth_consent_deny]") |> render_click()
      ...
      refute Bonfire.OpenID.Web.Consent.consented?(c.user, c.client.id, params(c, @protocol)["scope"])
    end
  end
  ...
  defp params(c, protocol) do
    %{"client_id" => c.client.id, "redirect_uri" => c.callback,
      "response_type" => "code", "state" => "state & + / café",
