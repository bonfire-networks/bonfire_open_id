defmodule Bonfire.OpenID.Web.MastoAvatarVisibilityApiTest do
      ...
      Map.new([:author, :reader, :outsider], fn role ->
        ...
        {:ok, token} =
          Boruta.Ecto.AccessTokens.create(
            %{client: Boruta.Ecto.OauthMapper.to_oauth_schema(client), sub: user.id, scope: "read write follow"},
            []
          )
  ...
  test "status accounts preserve uploaded avatars", %{author: author, reader: reader} do
    ...
    account = author |> api_conn() |> get("/api/v1/accounts/verify_credentials") |> json_response(200)
    refute account["avatar"] =~ "/images/avatar.png"
    status = publish(author, "public")
    response = reader |> api_conn() |> get("/api/v1/statuses/#{status["id"]}") |> json_response(200)

    assert response["account"]["avatar"] == account["avatar"]
    assert response["account"]["avatar_static"] == account["avatar_static"]
  end

  test "unlisted keeps its visibility and is readable by an outsider", %{author: author, outsider: outsider} do
    status = publish(author, "unlisted")
    response = outsider |> api_conn() |> get("/api/v1/statuses/#{status["id"]}") |> json_response(200)

    assert status["visibility"] == "unlisted"
    assert response["visibility"] == "unlisted"
  end

  test "private admits followers and excludes outsiders", %{author: author, reader: reader, outsider: outsider} do
    reader |> api_conn() |> post("/api/v1/accounts/#{author.user.id}/follow", %{}) |> json_response(200)
