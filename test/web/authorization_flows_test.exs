defmodule Bonfire.OpenID.Web.AuthorizationFlowsTest do
  ...
  setup tags do
    ...
    {:ok, account: account, user: user, client: client, conn: conn(user: user, account: account)}
  end

  for protocol <- ["oauth", "openid"] do
    @protocol protocol

    test "#{protocol}: first consent issues a code, token exchange works and consent is remembered",
         c do
      path = authorization_path(@protocol, c.client)
      callback = approve(c.conn, path)

      code =
        callback |> URI.parse() |> Map.fetch!(:query) |> URI.decode_query() |> Map.fetch!("code")
    ...
    test "#{protocol}: magic-link first consent preserves the authorization request", c do
      ...
      login =
        conn()
        |> get("/login/forgot-password/#{email.confirm_token}?go=#{URI.encode_www_form(path)}")
        ...
        "Authorization profile probe: " <>
          inspect(%{
            ...
            location_path:
              ...
              |> List.first()
              |> then(fn location -> if location, do: URI.parse(location).path end),
            account_loaded: is_map(probe.assigns[:current_account]),
            user_loaded: is_map(probe.assigns[:current_user])
          })
      )
    ...
    test "#{protocol}: unregistered callback cannot receive a code", c do
      response =
        c.conn
        |> get(
          authorization_path(@protocol, c.client, %{
            "redirect_uri" => "https://unregistered.example/callback"
          })
        )

      refute Enum.any?(
               get_resp_header(response, "location"),
               &String.starts_with?(&1, "https://unregistered.example")
             )
    ...
    test "#{protocol}: PKCE code exchange rejects the wrong verifier and accepts the correct one",
      ...
      bad =
        ...
        |> post(
          "/#{@protocol}/token",
          Map.put(token_params(c.client, code), "code_verifier", String.duplicate("x", 43))
        )
    ...
    test "#{protocol}: implicit authorization returns tokens in the callback fragment", c do
      ...
      {:ok, view, _} =
        live(c.conn, authorization_path(@protocol, c.client, %{"response_type" => type}))
  ...
  test "openid: prompt login redirects a signed-in user to reauthenticate", c do
    callback =
      c.conn
      |> get(authorization_path("openid", c.client, %{"prompt" => "login"}))
      |> redirected_to()
  ...
  defp authorization_path(protocol, client, extra \\ %{}) do
    params =
      %{
        ...
        "scope" => if(protocol == "openid", do: "openid profile email", else: "read write"),
        "state" => "review-state",
        "nonce" => "review-nonce"
      }
      |> Map.merge(extra)
  ...
  defp exchange(protocol, client, code, extra \\ %{}) do
    build_conn()
    |> post("/#{protocol}/token", Map.merge(token_params(client, code), extra))
    |> json_response(200)
  end
end
